...
Сделано:
* Реализация протокола Stop and Wait (все пункты, 8 баллов)
* Контрольные суммы (2 балла)

Файлы:
* rdt.py - реализация протокола rdt
* test_rdt.py - пример использования / тестирование реализации rdt
* check_sums.py - функции для контрольных сумм
...
В файле test_rdt.py создаются 2 связанных друг с другом rdt сокета, работающих поверх udp. Между сокетами в обе стороны одновременно передаются данные файла и проверяется, что они не изменились в дороге.

Там же моделируется потеря пакетов.

<img src="screens/rdt.png">
...
<img src="screens/check_sums.png">
