...
* трассировка с помощью ICMP (8 баллов)
...
Запуск:
```
sudo python traceroute.py <хост> <число запросов>
```

<img src="screens/traceroute.png">
...
Запуск клиента:
```
python client.py <хост> <порт>
```

Клиент читает всё из стандартного потока ввода и отправляет серверу,
затем ждёт получения сообщения такой же длины и выводит его в консоль.

<img src='screens/ipv6_client.png'>
...
Сервер слушает все соединения на данном порту и отправляет назад 
полученные сообщения в верхнем регистре (ограничение на размер сообщения - 1500 байт).

<img src='screens/ipv6_server.png'>
