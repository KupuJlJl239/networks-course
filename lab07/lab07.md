...
Сделано:
* всё про TCP (5+2 балла)
* задача про размер окна GBN/SR (3 балла)
...
2. Каков IP-адрес у сервера gaia.cs.umass.edu? Каковы номера портов для отправки и приема
   ...
   <img src="screens/ip_and_ports.png">
...
3. Какой порядковый номер у SYN TCP-сегмента, который используется для установления
   ...
   Относительный номер равен 0, абсолютный (который есть в заголовке TCP) - 212828396. Этот сегмент является SYN-сегментом из-за наличия соответствующего флага в заголовке TCP.

   <img src="screens/syn.png">
...
4. Какой порядковый номер у SYNACK-сегмента, отправленного сервером gaia.cs.umass.edu
   ...
   Относительный номер равен 0, абсолютный (который есть в заголовке TCP) - 3544253111. Значение в поле подтверждения - следующее после того номера, который сервер получил из нашего первого SYN-сегмента, то есть 212828397. Это SYNACK сегмент из-за наличия флагов SYN и ACK в заголовке.

   <img src="screens/synack.png">
...
5. Какой порядковый номер у TCP-сегмента, содержащего команду POST протокола HTTP?
   ...
   Относительный - 1, абсолютный - 212828397.
...
Все моменты времени указаны в секундах от начала соединения (взято из поля wireshark).

Номера сегментов относительные, чтобы получить реальные нужно прибавить 212828396.


В графе ACK указано, когда пришло подтверждение с таким номером. То есть чтобы посмотреть, когда сегмент реально подтвердился, нужно найти ближайшее значение в графе ACK в следующих строчках (но не в этой!).

| Номер сегмента | Время отправки | Время получения ACK |
|----------|----------|----------|
| 1        | 2.07513  |    -     |
| 632      | 2.07599  | 2.204067 |
| 2092     | 2.07601  |    -     |
| 3552     | 2.07608  | 2.204068 |
| 5012     | 2.07609  |    -     |
| 6472     | 2.07615  | 2.204068 |

   В среднем RTT (SampleRTT) = 0.128 секунды


   

   <img src="screens/data-packet.png">
   <img src="screens/ack-packet.png">
...
7. Чему равна пропускная способность (количество байтов, передаваемых в единицу
   ...
   По пакетам видно, что сначало было отправлено 13772 байта (около 2.07 секунд от начала), а потом мы ждали подтверждений, которые пришли около 2.20 секунд от начала.

   Итого пропускная способность в начальный момент времени была примерно $\frac{13772байт}{0.128сек} = 107593 бит/с = 1.08 Мбит/с$.

   <img src="screens/tcp-session.png">
...
#### Решение
Докажем утверждение: для корректной работы GBN или SR с окном размера $n$ сегментов необходимо и достаточно $2n$ номеров.

*Необходимость*. Рассмотрим ситуацию, когда отправитель посылает $n$ сегментов (с номерами 1, 2, ... , n), они все доходят в правильном порядке, получатель отправляет $n$ подтверждений (это так и в SR и в GBN), но все они не доходят. После этого окно отправителя не сдвинулось, а окно получателя сдвинулось на $n$ вперёд. Теперь отправитель по тайм-ауту переотправляет все пакеты. У получателя этих номеров не должно быть в окне, иначе он сочтёт что это новые порции данных. Таким образом, у отправителя и получателя у каждого по $n$ номеров в окнах, но все они разные, то есть должно быть минимум $2n$ номеров.

*Достаточность*. Заметим, что между окнами отправителя и получателя не может быть зазора: то есть последний номер в окне отправителя либо есть в окне получателя, либо следующий за ним - первый в окне получателя. Действительно, если есть зазор, то пакеты из зазора с одной стороны были получены (т.к. уже вышли из окна получателя), с другой - не могли быть отправлены (так как даже не входят в окно отправителя), так не бывает. Таким образом, если число номеров $k >= 2n$, то коллизий номеров возникнуть не может, значит и GBN и SR будут работать корректно.

Итак, если $k$ - число номеров, то максимальный размер окна равен $[k/2]$ ([] - округление в меньшую сторону).
